d3.csv(csvDataUrl).then(data => {
    ...
    // Lọc dữ liệu chỉ lấy các dòng có Mã PKKH là "B1"
    const filteredData = data.filter(d => d["Mã PKKH"] === "B1");

    if (filteredData.length === 0) {
        console.error("Không có dữ liệu nào với Mã PKKH = B1");
        d3.select("#chartQ11").append("p").text("Không có dữ liệu để hiển thị cho Mã PKKH = B1");
        return;
    }
    ...
    svg2.append("text")
        ...
        .attr("font-size", "18px")
        .attr("fill", "#337ab7")
        .text("Doanh số bán hàng theo Nhóm hàng của PKKH B1");
