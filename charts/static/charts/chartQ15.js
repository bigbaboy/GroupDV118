d3.csv(csvDataUrl).then(data => {
    ...
    // Lọc dữ liệu chỉ lấy các dòng có Mã PKKH là "C2"
    const filteredData = data.filter(d => d["Mã PKKH"] === "C2");

    if (filteredData.length === 0) {
        console.error("Không có dữ liệu nào với Mã PKKH = C2");
        d3.select("#chartQ15").append("p").text("Không có dữ liệu để hiển thị cho Mã PKKH = C2");
        return;
    }
    ...
    svg2.append("text")
        ...
        .attr("font-size", "18px")
        .attr("fill", "#337ab7")
        .text("Doanh số bán hàng theo Nhóm hàng của PKKH C2");
