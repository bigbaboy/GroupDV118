d3.csv(csvDataUrl).then(data => {
    ...
    // Lọc dữ liệu chỉ lấy các dòng có Mã PKKH là "C3"
    const filteredData = data.filter(d => d["Mã PKKH"] === "C3");

    if (filteredData.length === 0) {
        console.error("Không có dữ liệu nào với Mã PKKH = C3");
        d3.select("#chartQ16").append("p").text("Không có dữ liệu để hiển thị cho Mã PKKH = C3");
        return;
    }
    ...
    svg2.append("text")
        ...
        .attr("font-size", "18px")
        .attr("fill", "#337ab7")
        .text("Doanh số bán hàng theo Nhóm hàng của PKKH C3");
