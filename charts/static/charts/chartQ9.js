d3.csv(csvDataUrl).then(data => {
    ...
    // Lọc dữ liệu chỉ lấy các dòng có Mã PKKH là "A2"
    const filteredData = data.filter(d => d["Mã PKKH"] === "A2");

    if (filteredData.length === 0) {
        console.error("Không có dữ liệu nào với Mã PKKH = A2");
        d3.select("#chartQ9").append("p").text("Không có dữ liệu để hiển thị cho Mã PKKH = A2");
        return;
    }
    ...
    svg2.append("text")
        ...
        .attr("font-size", "18px")
        .attr("fill", "#337ab7")
        .text("Doanh số bán hàng theo Nhóm hàng của PKKH A2");
